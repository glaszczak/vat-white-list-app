...
app.get('/nip/', async (req, res) => {
    let url = `https://wl-api.mf.gov.pl/api/search/nip/${req.query.nipInput}?date=2019-10-06`;
    await axios.get(url)
        .then((resp) => {
            console.log(resp.data.result.subject)
            res.send(resp.data.result.subject)
        ...
        file.mv(`./public/${filename}`, async (err, ) => {
            if (err) {
                ...
                try {
                    ...
                    let resData = fs.readFileSync(`/public/nips.txt`, "utf-8").split("\r\n");
