...
async function gedDataFetch(nip) {
  //const res = await fetch(`https://wl-api.mf.gov.pl/api/search/nip/${nip}?date=2019-10-01`)
  let promise = await fetch(`https://wl-api.mf.gov.pl/api/search/nip/${nip}?date=2019-10-01`)
  //let promise = fetch(`https://github.com/`)
  promise.then((res) => {
    return res.json()
  }).then((json) => {
