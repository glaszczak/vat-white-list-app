    ...
    const promises = resData.map(async nip => {
        const response = await axios({
            ...
            if (resulted.status == 200) {
                return {
                    nip: nip,
                    name: resulted.data.result.subject.name,
                    residenceAddress: resulted.data.result.subject.residenceAddress,
                    workingAddress: resulted.data.result.subject.workingAddress,
                    status: resulted.data.result.subject.statusVat,
                    accountNumbers: resulted.data.result.subject.accountNumbers
                }
            }
        })
