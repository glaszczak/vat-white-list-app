...
app.get('/getJSON', async (req, res) => {
    ...
    let finalArray = resData.map(async (value) => {
        const result = await axios.get(url).then(result => result.data.result.subject.name)
        finalValue.name = result.name
        return finalValue
    })

    const resolvedFinalArray = await Promise.all(finalArray)
    //return functionThanUsesResolvedValues(resolvedFinalArray)
